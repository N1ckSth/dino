...
start_img = pygame.image.load('D:/VS Code/algo projects/dino/Assets/start_img.png')
start_img = pygame.transform.scale(start_img, (60, 64))

game_over_img = pygame.image.load('D:/VS Code/algo projects/dino/Assets/game_over.png')
game_over_img = pygame.transform.scale(game_over_img, (200, 36))

replay_img = pygame.image.load('D:/VS Code/algo projects/dino/Assets/replay.png')
replay_img = pygame.transform.scale(replay_img, (40, 36))
replay_rect = replay_img.get_rect()
replay_rect.x = WIDTH // 2 - 20
replay_rect.y = 100

numbers_img = pygame.image.load('D:/VS Code/algo projects/dino/Assets/numbers.png')
numbers_img = pygame.transform.scale(numbers_img, (120, 12))

# SOUNDS *********************************************************************

jump_fx = pygame.mixer.Sound('D:/VS Code/algo projects/dino/Sounds/jump.wav')
die_fx = pygame.mixer.Sound('D:/VS Code/algo projects/dino/Sounds/die.wav')
checkpoint_fx = pygame.mixer.Sound('D:/VS Code/algo projects/dino/Sounds/checkPoint.wav')
...
# GM -> immortal jutsu ( can't die )
# DM -> Swap between day and night
# LGM -> automatic jump and duck
# RICH -> add 10,000 to score
# HSC -> highscore is 99999
# SPDP -> increase speed by 2
...
while running:
	...
	for event in pygame.event.get():
		...
		if event.type == pygame.KEYDOWN:
			...
			keys.append(key)
			keys = keys[-7:]
			if ''.join(keys).upper() == 'GM':
				GODMODE = not GODMODE

			if ''.join(keys).upper() == 'DM':
				DAYMODE = not DAYMODE

			if ''.join(keys).upper() == 'LGM':
				LYAGAMI = not LYAGAMI

			if ''.join(keys).upper() == 'SPDP':
				SPEED += 2

			if ''.join(keys).upper() == 'RICH':
				score += 10000

			if ''.join(keys).upper() == 'HSC':
				high_score = 99999
